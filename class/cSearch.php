...
class CSearch{
	...
	function checkForSearch(){
		...
		if(isset($_POST["dni"]) && $this->cual == ""){
			...
			$this->where .= $this->checkFilter($this->where, "provincia", "p");
			$this->where .= $this->checkFilter($this->where, "autonomia", "p");
			$this->where .= $this->checkFilter($this->where, "cnae", "cnae");
			if($this->cual == ""){
	...
	function showTable($gene){
		if(isset($this->where) && $this->where != ""){
			$rows = $this->dbc->runSelect("consumidor c INNER JOIN provincias p ON c.provincia = p.id LEFT JOIN cnae ON cnae.id = c.cnae", $this->where, "c.*,  p.provincia as pr, p.autonomia as aut, cnae.cnae as cnaedesc", false, "100", false, false, false, false, 0);
		}else{
			$rows = Array();
		}
	...
	function showTableEmp($gene){
		...
		if(isset($this->where) && $this->where != ""){
			$limit = "200";
			$rows = $this->dbc->runSelect("empresario c INNER JOIN provincias p ON c.provincia = p.id LEFT JOIN empresario_puestos ep ON ep.id_empresario = c.id LEFT JOIN cnae ON cnae.id = c.cnae", 
			                              $this->where, 
										  "c.*, p.provincia as pr, p.autonomia as aut, cnae.cnae as cnaedesc, IF(cnae.porcentaje_admin IS NULL, cnae.porcentaje_auto, cnae.porcentaje_admin) as porcentaje, 
