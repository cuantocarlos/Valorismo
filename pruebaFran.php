...
<head>
    ...
    <style>
        ...
        .formwrapper select, .formwrapper input {
            margin: 5px 0;
            padding: 5px;
            width: 100%;
        }
...
<body>
    ...
    <form method="POST">
        <div class="formwrapper">
            ...
            <select name="cnae" id="cnae">
                <option value="">Seleccione un CNAE</option>
                <!-- Opciones generadas dinámicamente -->
                <option value="1">1 - Agricultura</option>
                <option value="2">2 - Industria</option>
            </select>
        </div>
        <div class="formwrapper">
            <label for="provincia">Ámbito</label>
    ...
    <?php
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cnae']) && $_POST['cnae'] !== '') {
    $cnae = $_POST['cnae'];
    $provincia = isset($_POST['provincia']) ? $_POST['provincia'] : 'Toda España';

    // Simulación de datos
    $valor_empresarial = 1000000;
    $coste_lab_med = 30000;
    $num_trabajadores = 50;
    $porc = 10;

    $valor_social = ($valor_empresarial * $porc) / 100;
    $resultado = round(($valor_empresarial * $porc) / 100 / $coste_lab_med);
    $total_trabajadores = $num_trabajadores + $resultado;
    $porc_trabajadores = round(($resultado * 100) / $total_trabajadores, 2);

    echo "<h3>Resultados para el CNAE $cnae en $provincia</h3>";
    echo "<table class='table'>
            <thead>
                <tr>
                    <th>Valor Empresarial Total</th>
                    <th>% Valor Social del Sector</th>
                    <th>Valor Social</th>
                    <th>Coste Laboral Medio</th>
                    <th>Incremento / Decremento Trabajadores</th>
            ...
            <tbody>
                <tr>
                    <td>$valor_empresarial €</td>
                    <td>$porc %</td>
                    <td>$valor_social €</td>
                    <td>$coste_lab_med €</td>
                    <td>$resultado</td>
                    <td>$total_trabajadores</td>
                    <td>$porc_trabajadores %</td>
                </tr>
            </tbody>
        </table>";
}
