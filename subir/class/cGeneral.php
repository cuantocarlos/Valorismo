...
class cGeneral {
	...
	function showMessageP($msg, $type="error", $close = true){
		if($type == "error"){
			$alert = " alert-error";
			$name = "Error: ";
		}elseif($type == "info"){
		...
		?>
		<div class="alert alert-block <?php echo $alert; ?>" style="padding-top: 0px; padding-bottom: 0px;">
			  <?php if($close){ ?><button data-dismiss="alert" class="close" type="button">&times;</button><?php } ?>
			  <p style="margin: 8px 0"><?php echo $msg; ?></p>
		</div><!--alert-->
		<?php
	...
	function creardb($name) {
		// Validar el nombre de la base de datos (solo letras, números y guiones bajos)
		if (!preg_match('/^[a-zA-Z0-9_]+$/', $name)) {
			return "nombre inválido";
		}
	
		// Crear conexión
		$enlace = new mysqli('localhost', 'tuintra', 'peportics');
		...
		if ($enlace->query($sql) === TRUE) {
			// Dar permisos al usuario tuintra
