...
if (isset($_POST["guardarprofile"])) {
	if (isset($_POST["titulo"]) && strlen($_POST["titulo"]) > 0) $data["titulo"] = $sql->fstr($_POST["titulo"]);
	else $err[0] = "Ha de introducir un título";
	if (isset($_POST["tipo_documento"]) && strlen($_POST["tipo_documento"]) > 0) $data["tipo_documento"] = $sql->fstr($_POST["tipo_documento"]);
	else $err[1] = "Ha de indicar el tipo de documento";
	if (count($err) > 0) {
		$msg = "";
		for ($x = 0; $x <= 2; $x++) {
			if ($msg != "") $space = "<br />";
...
function saveAndUpload($upload, $gene, $sql, $data)
{
	...
	if (isset($_FILES['archivo_upl']['name']) && $_FILES['archivo_upl']['name'] != "" && $data["id_consumidor"] != 0) {
		...
		if ($arr["errori"] > 0) {
			$error_arrayi = array(
				0 => "Archivo subida correctamente",
